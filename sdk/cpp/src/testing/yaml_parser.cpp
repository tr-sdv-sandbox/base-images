...
TestStep YamlParser::parse_step(const YAML::Node& node) {
    ...
    if (node["inject"]) {
        ...
        if (inject_node["actuator_mode"]) {
            std::string mode_str = inject_node["actuator_mode"].as<std::string>();
            data.mode = (mode_str == "target") ? sdv::vss::ActuatorMode::TARGET : sdv::vss::ActuatorMode::ACTUAL;
        }
        ...
        if (expect_node["actuator_mode"]) {
            std::string mode_str = expect_node["actuator_mode"].as<std::string>();
            data.mode = (mode_str == "target") ? sdv::vss::ActuatorMode::TARGET : sdv::vss::ActuatorMode::ACTUAL;
        }
