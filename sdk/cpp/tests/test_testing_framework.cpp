...
test_suite:
  ...
  test_cases:
    ...
    EXPECT_EQ(inject.mode, ActuatorMode::ACTUAL);
}
...
TEST_F(TestingFrameworkTest, ParseActuatorModes) {
    std::string yaml = R"(
test_suite:
  name: "Actuator Modes Test"
  ...
  test_cases:
    ...
    auto path = create_yaml_file("modes.yaml", yaml);
    YamlParser parser;
    auto suite = parser.parse_file(path);

    auto inject0 = std::get<InjectData>(suite.test_cases[0].steps[0].data);
    EXPECT_EQ(inject0.mode, ActuatorMode::TARGET);

    auto inject1 = std::get<InjectData>(suite.test_cases[0].steps[1].data);
    EXPECT_EQ(inject1.mode, ActuatorMode::ACTUAL);
}
...
TEST(TestModelsTest, TestStepVariant) {
    ...
    step.data = InjectData{"Vehicle.Speed", 50.0, ActuatorMode::ACTUAL};
