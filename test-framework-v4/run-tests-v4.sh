...
Run test framework v4 against a user function Docker image
...
# Start KUKSA databroker using the base-images docker-compose
print_info "Starting KUKSA databroker..."
docker-compose -f "$SCRIPT_DIR/../docker-compose.yml" up -d kuksa-databroker
...
for test_file in "${TEST_FILES[@]}"; do
    ...
    # Build test framework v4 command
    TEST_FRAMEWORK_CMD=(
        docker run --rm
        --network base-images_sdv-test
        ...
        test-framework-v4:latest
        python -m kuksa_test.cli
        /test-scenario.yaml
        --kuksa-url kuksa-databroker:55555
        --output /results/test-report.json
        --format json
    )
    ...
    if [ "$FAIL_FAST" = true ]; then
        TEST_FRAMEWORK_CMD+=(--stop-on-failure)
    fi
    
    # Run the test and also output to console
    if [ "$VERBOSE" = true ]; then
        print_info "Test command: ${TEST_FRAMEWORK_CMD[*]}"
    fi
    
    # Run with console output in addition to JSON report
    "${TEST_FRAMEWORK_CMD[@]}"
    TEST_EXIT_CODE=$?
    
    # Also run with console output for real-time feedback
    docker run --rm \
        --network base-images_sdv-test \
        -v "$(realpath "$test_file"):/test-scenario.yaml:ro" \
        -v /var/run/docker.sock:/var/run/docker.sock:ro \
        ...
        --kuksa-url kuksa-databroker:55555 \
        --format console \
        $([ ${#TAGS[@]} -gt 0 ] && printf -- '--tag %s ' "${TAGS[@]}") \
        $([ "$VERBOSE" = true ] && echo "--verbose") \
        $([ "$FAIL_FAST" = true ] && echo "--stop-on-failure")
